...
func (e *execution) ExecuteTextChat(input *structpb.Struct) (*structpb.Struct, error) {
	...
	requester := ModelRequesterFactory(inputStruct, e.client)

	return requester.SendChatRequest()
}

func ModelRequesterFactory(input ai.TextChatInput, client *httpclient.Client) IChatModelRequester {
...
type IChatModelRequester interface {
	SendChatRequest() (*structpb.Struct, error)
}
...
func (r *O1ModelRequester) SendChatRequest() (*structpb.Struct, error) {
...
func (r *SupportJSONOutputModelRequester) SendChatRequest() (*structpb.Struct, error) {
...
func (r *ChatModelRequester) SendChatRequest() (*structpb.Struct, error) {
	return &structpb.Struct{}, nil
}
	...
	for i, msg := range input.Data.Messages {
		...
		for j, c := range msg.Contents {
			...
			if c.Type == "image-base64" {
				content[j]["image_url"] = ai.GetDataURL(c.ImageBase64)
			}
		}
