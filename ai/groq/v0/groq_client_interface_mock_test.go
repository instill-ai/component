...
//go:generate minimock -i github.com/instill-ai/component/ai/groq/v0.GroqClientInterface -o groq_client_interface_mock_test.go -n GroqClientInterfaceMock -p groq
	...
	if mmChat.ChatMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.c1 != nil && !minimock.Equal(*mm_want_ptrs.c1, mm_got.c1) {
				mmChat.t.Errorf("GroqClientInterfaceMock.Chat got unexpected parameter c1, want: %#v, got: %#v%s\n", *mm_want_ptrs.c1, mm_got.c1, minimock.Diff(*mm_want_ptrs.c1, mm_got.c1))
			}

		} else if mm_want != nil && !minimock.Equal(*mm_want, mm_got) {
			mmChat.t.Errorf("GroqClientInterfaceMock.Chat got unexpected parameters, want: %#v, got: %#v%s\n", *mm_want, mm_got, minimock.Diff(*mm_want, mm_got))
		}
...
func (m *GroqClientInterfaceMock) MinimockChatInspect() {
	for _, e := range m.ChatMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to GroqClientInterfaceMock.Chat with params: %#v", *e.params)
		}
	}
	...
	if m.ChatMock.defaultExpectation != nil && afterChatCounter < 1 {
		if m.ChatMock.defaultExpectation.params == nil {
			m.t.Error("Expected call to GroqClientInterfaceMock.Chat")
		} else {
			m.t.Errorf("Expected call to GroqClientInterfaceMock.Chat with params: %#v", *m.ChatMock.defaultExpectation.params)
		}
	}
	...
	if !m.ChatMock.invocationsDone() && afterChatCounter > 0 {
		m.t.Errorf("Expected %d calls to GroqClientInterfaceMock.Chat but found %d calls",
