...
| Chunk Setting | `strategy.setting` | [setting](#setting-details) | Chunk Setting |
...
<details>
<summary>
<strong> <a id="setting-details"></a>The <code>setting</code> object</strong>  
</summary>
</br>
<code>strategy.setting</code> must fulfill one of the following schemas:
...
| End Position | `text-chunks.end-position` | integer | The ending position of the chunk in the original text |
| Start Position | `text-chunks.start-position` | integer | The starting position of the chunk in the original text |
| Text | `text.chunks.text` | string | Text chunk after splitting |
| Token Count | `text-chunks.token-count` | integer | Count of tokens in a chunk |
