...
| Database Name (required) | `database-name` | string | Fill in your MongoDB database name |
| Collection Name (required) | `collection-name` | string | The name of the collection in the MongoDB database |
| Filter | `filter` | semi-structured/json | The filter to find documents, if JSON value of a key is null e.g \{'name':null\}, then name field will be selected without any filter. If empty, then all fields will be selected |
