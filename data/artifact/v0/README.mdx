...
| Role (required) | `role` | string | The role of the user who sends the message. Now, we support 'user' and 'assistant'. |
| Message Type (required) | `message-type` | string | The message type of the chat history. Now, we only support 'MESSAGE_TYPE_TEXT'. |
