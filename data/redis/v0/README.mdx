...
| SSL Configuration | `ssl-mode` | object | SSL connection modes.    <b>verify-full</b> - This is the most secure mode. Always require encryption and verifies the identity of the source database server |
...
| Input | ID | Type | Description |
| :--- | :--- | :--- | :--- |
| Task ID (required) | `task` | string | `TASK_WRITE_chat-message` |
| Session ID (required) | `session-id` | string | A unique identifier for the chat session |
...
| Input | ID | Type | Description |
| :--- | :--- | :--- | :--- |
| Task ID (required) | `task` | string | `TASK_WRITE_MULTI_MODAL_chat-message` |
| Session ID (required) | `session-id` | string | A unique identifier for the chat session |
| Role (required) | `role` | string | The message role, i.e. 'system', 'user' or 'assistant' |
