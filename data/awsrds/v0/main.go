...
// newClient creates a new SQL client with traditional username and password
...
// newClientIAM creates a new SQL client with IAM authentication
...
func (e *execution) Execute(ctx context.Context, inputs []*structpb.Struct) ([]*structpb.Struct, error) {
	...
	// check if the password field is present, if it is, create a client with username and password else create a client with IAM
	...
	for _, input := range inputs {
		...
		case taskUpsert:
			...
			var err error
			if ListValue, ok := input.GetFields()["data"].GetKind().(*structpb.Value_ListValue); ok {
				for _, data := range ListValue.ListValue.Values {
					err = UpsertUser(client, &tableName, data.GetStructValue())
					if err != nil {
						return nil, err
					}
				}
...
func UpsertUser(client *sqlx.DB, tableName *string, data *structpb.Struct) error {
	...
	for dataKey, dataValue := range data.GetFields() {
		...
		updateColumns = append(updateColumns, fmt.Sprintf("%s = VALUES(%s)", dataKey, dataKey))
