...
| Source Only | `source-only` | boolean | If true source only will return only documents, if false returns documents with (_id, _index, _score), default to true |
...
| Source Only | `source-only` | boolean | If true source only will return only documents, if false returns documents with (_id, _index, _score), default to false |
| Index Name (required) | `index-name` | string | Name of the Elasticsearch index |
...
| Index Name (required) | `index-name` | string | Name of the Elasticsearch index |
| Data (required) | `array-data` | array[object] | Array data to be indexed |
...
| Filter | `filter` | object | The query dsl filter which starts with "query" field, please refer to https://www.elastic.co/guide/en/elasticsearch/reference/current/query-filter-context.html |
| Filter SQL | `filter-sql` | string | The filter to be applied to the data with SQL syntax, which starts with WHERE clause, empty for no filter |
| Update (required) | `update` | object | Update data |
